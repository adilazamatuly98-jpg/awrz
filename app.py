...
def _generate_demo_data(db):
    ...
    # ── Номерные (с серийным номером) ──
    parts_avtoscepka_num = [
        ("Корпус автосцепки СА-3", "в оборот"),
        ("Тяговый хомут", "в оборот"),
        ("Поглощающий аппарат Ш-2-Т", "забраковано"),
        ("Поглощающий аппарат Ш-6-ТО-4", "в оборот"),
        ("Клин тягового хомута", "в оборот"),
        ("Упорная плита", "забраковано"),
    ]
    parts_avtoscepka_unnum = [
        ("Замок автосцепки", "забраковано"),
        ("Замкодержатель", "в оборот"),
        ("Предохранитель замка", "в оборот"),
        ("Подъёмник замка", "забраковано"),
        ("Валик подъёмника", "в оборот"),
        ("Центрирующая балочка", "в оборот"),
        ("Маятниковая подвеска", "в оборот"),
        ("Расцепной рычаг", "в оборот"),
        ("Цепочка расцепного привода", "забраковано"),
    ]
    parts_telega_num = [
    ...
    parts_telega_unnum = [
        ...
        ("Планка фрикционная подвижная М1698.02.004", "в оборот"),
        ("Планка фрикционная неподвижная М1698.02.001", "в оборот"),
        ("Пружина рессорная наружная 1272.3045858008", "в оборот"),
        ("Пружина рессорная внутренняя 1272.3045858007", "в оборот"),
        ("Колодка тормозная", "забраковано"),
        ("Триангель 100.40.010-2", "в оборот"),
        ("Чека 100-40-014", "в оборот"),
        ("Подвеска тормозного башмака", "в оборот"),
        ("Колпак скользуна М1698.01.100", "забраковано"),
    ]
    parts_akp_num = [
        ...
        ("Главная часть воздухораспределителя 270", "в оборот"),
        ("Магистральная часть воздухораспределителя 483", "в оборот"),
        ("Кран концевой", "в оборот"),
        ("Кран разобщительный", "забраковано"),
    ]
    parts_akp_unnum = [
        ("Манжета 270-313", "забраковано"),
        ("Манжета 305-156", "забраковано"),
        ("Манжета 270-397-3", "в оборот"),
        ("Прокладка 270-330-1", "забраковано"),
        ("Прокладка 270-326", "забраковано"),
        ("Прокладка 270-549", "в оборот"),
        ("Пружина 270.322-1", "в оборот"),
        ("Уплотнение 270-311", "в оборот"),
    ]
    parts_kuzov_num = [
        ("Хребтовая балка", "в оборот"),
        ("Шкворневая балка", "в оборот"),
        ("Концевая балка", "в оборот"),
    ]
    parts_kuzov_unnum = [
        ("Крышка люка", "забраковано"),
        ("Петля крышки люка", "в оборот"),
        ("Скоба крышки люка", "в оборот"),
        ("Боковая стойка", "забраковано"),
        ("Раскос", "в оборот"),
        ("Кронштейн", "в оборот"),
        ("Скоба", "в оборот"),
        ("Болт М22х90", "в оборот"),
        ("Гайка М22", "в оборот"),
    ]
    ...
    for i, num in enumerate(wagon_numbers):
        ...
        # Каждый участок получает И номерные, И неномерные детали в больших количествах
        for uch_name, num_pool, unnum_pool in [
            ("avtoscepka", parts_avtoscepka_num, parts_avtoscepka_unnum),
            ("telega",     parts_telega_num,     parts_telega_unnum),
            ("akp",        parts_akp_num,        parts_akp_unnum),
            ("kuzov",      parts_kuzov_num,      parts_kuzov_unnum),
        ]:
            oid = next_id(db, "opzs")
            ...
            # Номерные: 3-5 деталей
            num_count = min(len(num_pool), random.randint(3, 5))
            for pname, cond in random.sample(num_pool, num_count):
                p = {
                    "kind": "num",
                    "type": pname,
                    "number": f"{random.randint(10,99)}-{random.randint(1000,9999)}",
                    "number_after": f"{random.randint(10,99)}-{random.randint(1000,9999)}",
                    "zavod": random.choice(ZAVODY),
                    "god": str(random.randint(2015, 2024)),
                    "cond": cond,
                    "installed_from": random.choice(["из оборота", "новый", "давальческий"]),
                    "reject_reason": random.choice([
                        "Износ выше нормы", "Трещина по сварному шву",
                        "Деформация рабочей поверхности", "Излом по основному телу"
                    ]) if cond == "забраковано" else "",
                }
                # Для АКП также Тип/Класс
                if uch_name == "akp":
                    p["type_detail"] = random.choice(["Т-1", "Т-2", "Т-3"])
                    p["class_detail"] = random.choice(["Класс А", "Класс Б"])
                opzs_parts.append(p)

            # Неномерные: 4-7 деталей
            unnum_count = min(len(unnum_pool), random.randint(4, 7))
            for pname, cond in random.sample(unnum_pool, unnum_count):
                p = {
                    "kind": "unnum",
                    "type": pname,
                    ...
                    "installed_from": random.choice(["из оборота", "новый", "давальческий"]),
                    "reject_reason": random.choice([
                        "Поверхностные дефекты", "Износ выше допустимого",
                        "Срок службы истёк", "Не соответствует ТУ"
                    ]) if cond == "забраковано" else "",
                }
                opzs_parts.append(p)
            ...
            if opzs["status"] == "На доработке":
                opzs["comment"] = random.choice([
                    "Проверьте корректность номера после",
                    "Уточните завод-изготовитель для замка",
                    "Добавьте обоснование выбраковки",
                ])
            db["opzs"].append(opzs)
